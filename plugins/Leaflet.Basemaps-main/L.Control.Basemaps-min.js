(L.Control.Basemaps = L.Control.extend({
  ...
  onAdd: function (e) {
    ...
    return (
      ...
      this.options.basemaps.forEach(function (s, o) {
        var a,
          i = "basemap";
        if (
          (0 === o
            ? ((this.basemap = s), this._map.addLayer(s), (i += " active"))
            : 1 === o && (i += " alt"),
          s.options.iconURL)
        )
          a = s.options.iconURL;
          ...
          if (
            ...
            var c = s._tileCoordsToBounds(p),
              ...
              v = (m >= 1.3 && n === L.CRS.EPSG4326
                ? [h.y, d.x, d.y, h.x]
                : [d.x, h.y, h.x, d.y]
              ).join(",");
            a +=
              L.Util.getParamString(r, a, s.options.uppercase) +
              (s.options.uppercase ? "&BBOX=" : "&bbox=") +
              v;
          }
        }
        ...
        (u.src = a),
          s.options && s.options.label && (u.title = s.options.label),
          L.DomEvent.on(
            b,
            "click",
            function () {
              if (
                this.options.basemaps.length > 2 &&
                L.Browser.mobile &&
                L.DomUtil.hasClass(t, "closed")
              )
                L.DomUtil.removeClass(t, "closed");
              else if (s != this.basemap) {
                e.removeLayer(this.basemap),
                  e.addLayer(s),
                  s.bringToBack(),
                  e.fire("baselayerchange", s),
                  (this.basemap = s),
                  L.DomUtil.removeClass(
                    t.getElementsByClassName("basemap active")[0],
                    "active"
                  ),
                  L.DomUtil.addClass(b, "active");
                var a = (o + 1) % this.options.basemaps.length;
                L.DomUtil.removeClass(
                  t.getElementsByClassName("basemap alt")[0],
                  "alt"
                ),
                  L.DomUtil.addClass(
                    t.getElementsByClassName("basemap")[a],
                    "alt"
                  ),
