  ...
  var a = (function () {
    return function () {
      (this.series = void 0),
        ...
        (this.breaks = void 0),
        (this.colorCode = void 0),
        (this.range = void 0),
        (this.statMethod = void 0),
        (this.colorSchemes = {
          OrRd: {
        ...
        (this.getColorCodes = function () {
          ...
          for (code in this.colorSchemes)
            this.colorSchemes.hasOwnProperty(code) && a.push(code);
          return a;
        }),
        (this.getColorCodesByType = function () {
          var a = {};
          for (code in this.colorSchemes)
            this.colorSchemes.hasOwnProperty(code) &&
              (a.hasOwnProperty(this.colorSchemes[code].properties.type) ||
                (a[this.colorSchemes[code].properties.type] = []),
              a[this.colorSchemes[code].properties.type].push(code));
          return a;
        }),
        ...
        (this._classifyQuantile = function () {
          for (
            var a = this.series.sort(function (a, b) {
                return a - b;
              }),
              b = [],
              c = this.series.length / this.getNumClasses(),
              d = 1;
            d < this.getNumClasses();
            d++
          ) {
            var e = Math.round(d * c + 0.49);
            b.push(a[e - 1]);
          }
          var f = b;
          return (
            f.unshift(a[0]),
            f[a.length - 1] !== a[a.length - 1] && f.push(a[a.length - 1]),
            (this.range = f),
            this.range.sort(function (a, b) {
              return a - b;
            }),
        ...
        (this._classifyStdDeviation = function () {
          ...
          if (this.getNumClasses % 2 == 1) {
            var d = Math.floor(this.getNumClasses() / 2),
              e = d + 1;
            for (
              c[d] = this._mean(this.series) - this._stdDev(this.series) / 2,
                c[e] = this._mean(this.series) + this._stdDev(this.series) / 2,
                i = d - 1;
              i > 0;
              i--
            ) {
        ...
        (this._classifyJenks = function () {
          for (var a = [], b = 0, c = this.series.length + 1; b < c; b++) {
            for (var d = [], e = 0, f = this.numClasses + 1; e < f; e++)
              d.push(0);
            a.push(d);
          }
          for (var g = [], h = 0, i = this.series.length + 1; h < i; h++) {
            for (var j = [], k = 0, l = this.numClasses + 1; k < l; k++)
              j.push(0);
            g.push(j);
          }
          for (var m = 1, n = this.numClasses + 1; m < n; m++) {
            (a[0][m] = 1), (g[0][m] = 0);
            for (var o = 1, p = this.series.length + 1; o < p; o++)
              g[o][m] = 1 / 0;
            var q = 0;
          }
          for (var r = 2, s = this.series.length + 1; r < s; r++) {
            for (var t = 0, u = 0, v = 0, w = 1, x = r + 1; w < x; w++) {
              var y = r - w + 1,
                z = parseFloat(this.series[y - 1]);
              (u += z * z), (t += z), (v += 1), (q = u - (t * t) / v);
              var A = y - 1;
              if (0 != A)
                for (var B = 2, C = this.numClasses + 1; B < C; B++)
                  g[r][B] >= q + g[A][B - 1] &&
                    ((a[r][B] = y), (g[r][B] = q + g[A][B - 1]));
            }
            (a[r][1] = 1), (g[r][1] = q);
          }
          var D = this.series.length,
            E = [];
          for (h = 0, i = this.numClasses + 1; h < i; h++) E.push(0);
          (E[this.numClasses] = parseFloat(
            this.series[this.series.length - 1]
          )),
            (E[0] = parseFloat(this.series[0]));
        ...
        (this.classify = function (a, b) {
          (this.statMethod = void 0 !== a ? a : this.statMethod),
            (this.numClasses = void 0 !== b ? b : this.numClasses);
          var c = void 0;
          switch (a) {
            case "equal_interval":
        ...
        (this.getColorInRange = function (a) {
          ...
          for (b; b < this.range.length; b++) {
            if (!(a >= this.range[b] && b < this.range.length))
              return (
                a == this.range[b] &&
                this.colorSchemes[this.colorCode][this.numClasses][b - 1]
              );
            if (a <= this.range[b + 1])
              return this.colorSchemes[this.colorCode][this.numClasses][b];
          }
        }),
